			...
			?>
				...
				else
				{
                	...
                	<div style="width:200px; float:left; margin:0px 20px 0px 0px; ">
                    	<center>
                          <?php echo "Monto:".number_format($suma[$s]["SUM(mont_bsf_trab)"], 2, ',', '.')." Bsf"; ?>
                          <?php echo "Total:".number_format($suma[$s]["SUM(tota_vef_trab)"], 2, ',', '.')." Bsf"; ?>
                        </center>
                    </div>
                    </div>
				 ...
				 for($i=0; $i<sizeof($paginacion); $i++)
                  {
                    ...
                    ?>
						...
						if ($row[$i]["id_usuario"] !=0)
						{
							...
							//Consulta la BD 		
							$respuesta=mysql_query("select * from tb_usuarios as A inner join tb_trabajos_asignados as B on A.id_usuario = B.id_usuario where A.id_usuario =  $id_usuario");
							while($sel =mysql_fetch_array($respuesta))
							{
							?>
